...
std::string file_path = "scratch/output/static";
...
{
  FlowMonitor::FlowStatsContainer stats = monitor->GetFlowStats (); // This container holds statistics about the flows being monitored.
...
int main (int argc, char *argv[])
{
  // Parse command line arguments
    ...
    mobility.SetPositionAllocator("ns3::GridPositionAllocator",
                                  ...
                                  DoubleValue(0.0),
                                  "DeltaX",
                                  DoubleValue(coverageArea/2), //coverageArea
                                  "DeltaY",
                                  DoubleValue(coverageArea/2),
                                  "GridWidth",
                                  UintegerValue(10),
                                  "LayoutType",
  ...
  if(whichParam == 1) {
    ...
    std::ofstream ofs_thp (file_path+"/throughput_areas.dat", std::ios::out | std::ios::app);
    std::ofstream ofs_deliver (file_path+"/deliveryR_areas.dat", std::ios::out | std::ios::app);
    ofs_thp <<  coverageArea << " " << avgThroughput << std::endl; // throughput (bit/s)
    ofs_deliver <<  coverageArea << " " << avgDelivery_ratio << std::endl; // delivery ratio (%)
